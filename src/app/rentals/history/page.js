...
export default function PlaystationHistory() {
    ...
    const { data: dataRental, isLoading, error } = useGetRentalDone(
    ...
    const handlePrintPDF = () => {
        ...
        const filteredDate = startDate ? dayjs(startDate).format('DD-MM-YYYY') : 'Semua';
        const branchName = branchOptions.find(b => b.value === selectedBranch)?.label || 'Semua';
        ...
        const tableColumn = [
            "ID", "Nama Lengkap", "Cabang",
            "Tipe PS", "Nomor PS", "Tipe Sewa",
            "Total Harga", "Status"
        ];
        ...
        const tableRows = dataRental.map(rental => [
            ...
            rental.ps_type,
            rental.ps_number,
            rental.rental_type === 'on-site' ? 'Ditempat' :
                rental.rental_type === 'take-home' ? 'Dibawa Pulang' : rental.rental_type,
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, px: 2, flexWrap: 'wrap', gap: 2 }}>
                            <div className="flex space-x-2 items-center">
                                <MultipleSelectChipBranch
                                    ...
                                    setPersonName={(selected) => setSelectedBranch(selected[0] || '')}
                                />
